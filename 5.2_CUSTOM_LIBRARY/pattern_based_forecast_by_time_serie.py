# make model, train, forecasts and register the MSE for a pattern_based approach
import os
import sys
import logging
...
def calculate_mse(local_cm_settings, local_pattern_based_approach_forecast, local_cm_forecasts, local_cm_ground_truth):
    ...
    for local_cm_time_serie in range(nof_cm_time_series):
        ...
        time_series_treated.append([int(local_cm_time_serie), int(local_pattern_based_approach_forecast),
    ...
    if os.path.isfile(''.join([local_cm_settings['models_evaluation_path'],
                               'pattern_based approach_time_series_results_model_mse.npy'])):
    ...
    np.savetxt(''.join([local_cm_settings['models_evaluation_path'], 'pattern_based approach_model_mse.csv']),
               time_series_treated, fmt='%10.15f', delimiter=',', newline='\n')
    np.save(''.join([local_cm_settings['models_evaluation_path'], 
                     'pattern_based_approach_time_series_results_model_mse']),
            time_series_treated)
    return True


def make_forecast_structure(local_mfs_settings):
    local_pattern_based_approach_forecast_mse = np.load(''.join([local_mfs_settings['models_evaluation_path'],
                                                'pattern_based approach_time_series_results_model_mse.npy']))
    ...
    for local_mfs_time_serie in range(local_nof_time_series):
        local_best_mse_y_pred[local_mfs_time_serie, :] = local_pattern_based_approach_forecast_mse[local_mfs_time_serie, 1]
    return local_best_mse_y_pred

# classes definitions


class generate_pattern_based_approach_forecast_and_calculate_mse:
        ...
        try:
            ...
            for fixed_forecast in range(local_min_sales, int(local_max_sales / 2)):
                ...
                if calculate_mse_review:
                    print('mse calculation for this pattern_based approach forecast done')
                else:
                    print('error at calculation for this pattern_based approach forecast')
            ...
            local_forecast_name = 'pattern_based approach_forecast_data'
            ...
            if save_submission_review:
                print('pattern_based approach forecast generation successfully completed')
            else:
                print('error at saving pattern_based approach forecast generation')

        except Exception as submodule_error:
            print('pattern_based approach forecast generation submodule_error: ', submodule_error)
            logger.info('error in pattern_based approach generation submodule')
