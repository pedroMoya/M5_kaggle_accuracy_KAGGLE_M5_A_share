...
def evaluate():
    try:
        ...
        y_pred_array = []
        # change later for time_series_not_improved_stochastic_simulation.npy (first needed to train that ts_model)
        time_serie_not_improved_with_first_model = \
            np.load(''.join([local_script_settings['models_evaluation_path'],
                             'time_series_not_improved.npy']))
        ...
        np.save(''.join([local_script_settings['models_evaluation_path'], '_y_pred_array_']),
                y_pred_array)
